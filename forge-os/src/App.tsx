...
type ProviderReadiness = {
  ...
  apify: ProviderStatus
}

type ContentDigestPack = {
  title: string
  platform: string
  ...
  scriptSegments: Array<{ label: string; seconds: string; content: string }>
  imagePrompt: string
  videoPrompt: string
  emailSubject: string
  emailBody: string
...
function CRM() {
  ...
  const [surfaceStatus, setSurfaceStatus] = useState('Loading CRM intelligence...')

  useEffect(() => {
    const load = async () => {
      try {
        const [dashboardResponse, prospectsResponse, outreachResponse, readinessResponse] = await Promise.all([
          apiFetch('/api/crm/dashboard'),
          apiFetch('/api/crm/prospects'),
          apiFetch('/api/outreach/dashboard?limit=12'),
          apiFetch('/signals/providers/readiness/?live=false'),
        ])
        ...
        if (outreachJson?.ok) {
          setActivity({
            drafts: outreachJson.drafts ?? [],
            sends: outreachJson.sends ?? [],
            replies: outreachJson.replies ?? [],
            source: outreachJson.source ?? 'unknown',
          })
        }
        setProviders({
          gmail: readinessJson?.gmail ?? { configured: false, status: 'not_configured' },
          apify: readinessJson?.apify ?? { configured: false, status: 'not_configured' },
        })
        setSurfaceStatus('CRM intelligence synced from backend surfaces.')
      } catch {
        setSurfaceStatus('CRM intelligence failed to load from the backend.')
      }
    }
    void load()
  }, [])
  ...
  const stageCounts = useMemo(() => {
    ...
    for (const prospect of prospects) {
      const key = (prospect.status ?? 'unknown').toLowerCase()
      counts.set(key, (counts.get(key) ?? 0) + 1)
    }
    return [
      { label: 'New', key: 'new', count: counts.get('new') ?? 0 },
      { label: 'Active', key: 'active', count: counts.get('active') ?? 0 },
      { label: 'Contacted', key: 'contacted', count: counts.get('contacted') ?? 0 },
      { label: 'Qualified', key: 'qualified', count: counts.get('qualified') ?? 0 },
      { label: 'Converted', key: 'converted', count: counts.get('converted') ?? 0 },
      { label: 'Unknown', key: 'unknown', count: counts.get('unknown') ?? 0 },
    ]
  }, [prospects])
  ...
  const sourceMix = useMemo(() => {
    const counts = new Map<string, number>()
    for (const prospect of prospects) {
      const key = prospect.source ?? 'unknown'
      counts.set(key, (counts.get(key) ?? 0) + 1)
    }
    ...
    return [...counts.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 4)
      .map(([source, count]) => ({ source, count, share: Math.round((count / total) * 100) }))
  }, [prospects])

  const filteredProspects = useMemo(() => {
    if (statusFilter === 'all') return prospects
    return prospects.filter((prospect) => (prospect.status ?? 'unknown').toLowerCase() === statusFilter)
  }, [prospects, statusFilter])

  const emailCoverage = useMemo(() => {
    if (prospects.length === 0) return 0
    return Math.round((prospects.filter((prospect) => Boolean(prospect.email)).length / prospects.length) * 100)
  }, [prospects])

  return (
    <div>
      <PageHeader title="AI CRM" right={<span className="pill red"><Bot size={11} /> Jarvis Sales Command</span>} />
      <div className="crm-command">
        <div>
          <p className="crm-kicker">Trust Tax Countermeasure</p>
          <h3>Run one surface for leads, outreach, content, and operator readiness.</h3>
          <p>{surfaceStatus}</p>
        </div>
        <div className="crm-command-tags">
          <span className="pill">{activity?.source === 'supabase_live' ? 'SUPABASE LIVE' : activity?.source ?? 'UNKNOWN SOURCE'}</span>
          <span className="pill">{providers?.gmail.configured ? 'GMAIL READY' : 'GMAIL BLOCKED'}</span>
          <span className="pill">{providers?.apify.configured ? 'SOURCING READY' : 'SOURCING BLOCKED'}</span>
        </div>
      </div>

      <div className="crm-metric-grid">
        <div className="stat crm-stat"><p>Total prospects</p><h3>{summary?.totalProspects ?? 0}</h3><span>Lead inventory across the CRM.</span></div>
        <div className="stat crm-stat"><p>Active prospects</p><h3>{summary?.activeProspects ?? 0}</h3><span>Leads still in motion.</span></div>
        <div className="stat crm-stat"><p>Email coverage</p><h3>{emailCoverage}%</h3><span>Prospects with usable inbox data.</span></div>
        <div className="stat crm-stat"><p>Drafted outreach</p><h3>{activity?.drafts.length ?? 0}</h3><span>Recent outbound drafts in queue.</span></div>
        <div className="stat crm-stat"><p>Recent replies</p><h3>{activity?.replies.length ?? 0}</h3><span>Latest inbound intent signals.</span></div>
        <div className="stat crm-stat"><p>Industries</p><h3>{summary?.industries.length ?? 0}</h3><span>Distinct ICP slices loaded.</span></div>
      </div>

      <div className="crm-grid">
        <div className="card">
          <CardTitle title="Jarvis Control Loop" />
          <div className="crm-brief-grid">
            <div className="crm-brief-panel">
              <span className="crm-panel-label">Next operator moves</span>
              <div className="crm-action-list">
                <div className="crm-action-item">
                  <strong>Push authority content daily</strong>
                  <span>{activity?.drafts.length ?? 0} recent drafts and {activity?.replies.length ?? 0} recent replies are available to turn into proof posts.</span>
                </div>
                <div className="crm-action-item">
                  <strong>Protect inbox readiness</strong>
                  <span>{providers?.gmail.configured ? `Gmail sender ${providers.gmail.sender ?? 'configured'} is available.` : 'Gmail is not configured, so reply sync and digest drafts are blocked.'}</span>
                </div>
                <div className="crm-action-item">
                  <strong>Keep cold traffic warm</strong>
                  <span>{sourceMix.length > 0 ? `Top source is ${sourceMix[0].source} at ${sourceMix[0].share}% of visible leads.` : 'No source mix has populated yet.'}</span>
                </div>
              </div>
            </div>
            <div className="crm-brief-panel">
              <span className="crm-panel-label">Provider readiness</span>
              <div className="crm-provider-stack">
                <div className={`crm-provider-card ${providers?.gmail.configured ? 'ready' : 'blocked'}`}>
                  <strong>Gmail</strong>
                  <span>{providers?.gmail.configured ? providers?.gmail.status ?? 'configured' : 'not configured'}</span>
                  <small>{providers?.gmail.configured ? 'Used for reply sync and content digest drafts.' : 'Needs Google OAuth client, secret, refresh token, and sender email.'}</small>
                </div>
                <div className={`crm-provider-card ${providers?.apify.configured ? 'ready' : 'blocked'}`}>
                  <strong>Apify / sourcing</strong>
                  <span>{providers?.apify.configured ? providers?.apify.status ?? 'configured' : 'not configured'}</span>
                  <small>{providers?.apify.configured ? 'Lead sourcing provider is wired.' : 'Lead ingestion can still run through manual import or n8n search flows.'}</small>
                </div>
                <div className="crm-provider-card warning">
                  <strong>IG / FB local outreach</strong>
                  <span>manual runtime verification required</span>
                  <small>These channels are browser/session driven and must be treated separately from backend API readiness.</small>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="card">
          <CardTitle title="Stage Radar" />
          <div className="crm-stage-board">
            {stageCounts.map((stage) => (
              <button
                key={stage.key}
                className={`crm-stage-tile ${statusFilter === stage.key ? 'active' : ''}`}
                onClick={() => setStatusFilter(statusFilter === stage.key ? 'all' : stage.key)}
              >
                <p>{stage.label}</p>
                <h3>{stage.count}</h3>
                <span>{statusFilter === stage.key ? 'Showing stage' : 'Filter stage'}</span>
              </button>
            ))}
          </div>
          <div className="crm-source-mix">
            <span className="crm-panel-label">Source concentration</span>
            {sourceMix.length === 0 && <p className="crm-empty">No source data is visible yet.</p>}
            {sourceMix.map((item) => (
      ...
      <div className="card">
        <CardTitle title="Live Lead Grid" />
        <div className="crm-toolbar">
          <div className="segmented">
            <button className={statusFilter === 'all' ? 'active' : ''} onClick={() => setStatusFilter('all')}>All</button>
            {stageCounts.map((stage) => (
              <button key={stage.key} className={statusFilter === stage.key ? 'active' : ''} onClick={() => setStatusFilter(stage.key)}>
                {stage.label}
              </button>
            ))}
          </div>
          <span>{filteredProspects.length} visible records</span>
        </div>
        <table>
          <thead><tr><th>Name</th><th>Company</th><th>Email</th><th>Industry</th><th>Status</th><th>Source</th></tr></thead>
          <tbody>
            {filteredProspects.length === 0 && <tr><td colSpan={6}>No real prospects are loaded yet.</td></tr>}
            {filteredProspects.slice(0, 30).map((prospect) => (
              <tr key={prospect.id}>
                ...
                <td><span className="badge crm-badge">{prospect.status ?? 'unknown'}</span></td>
...
function ContentCycle() {
  ...
  const [digestEmail, setDigestEmail] = useState('jacepersonalai@gmail.com')
  const [loading, setLoading] = useState(false)
  const [digesting, setDigesting] = useState(false)
  const [status, setStatus] = useState('')
  const [digestPack, setDigestPack] = useState<ContentDigestPack | null>(null)
  ...
  const draftContentPack = async () => {
    setDigesting(true)
    setStatus('Jarvis is packaging a 20-second reel brief and drafting the Gmail handoff...')
    try {
      const response = await apiFetch('/api/cmo/digest', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          niche,
          ...
          durationSeconds: 20,
          goal: 'Create short-form authority content that makes Virel look trusted before outbound touches hit.',
        }),
      })
      const json = await response.json()
      if (json?.ok) {
        setDigestPack(json.pack ?? null)
        if (json.gmail?.draftCreated) {
          setStatus(`Content pack drafted to Gmail for ${digestEmail}. Draft ID: ${json.gmail.draftId}`)
        } else if (json.gmail?.configured === false) {
          setStatus('Content pack generated, but Gmail is not configured in the backend yet.')
        } else {
          setStatus('Content pack generated.')
        }
      } else {
        setStatus('Content pack generation failed.')
      }
    } catch (error) {
      setStatus(`Content pack generation failed: ${(error as Error).message}`)
    } finally {
      setDigesting(false)
    }
  }
            ...
            <div className="composer-row">
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <input
                  ...
                  placeholder="Digest email"
                  style={{ height: 32, background: '#0c0c0c', border: '1px solid #1c1c1c', color: '#fff', borderRadius: 5, padding: '0 8px', fontFamily: 'inherit', fontSize: 11, width: 220 }}
                />
                <button className="btn primary" onClick={generateFromCMO} disabled={loading}>
                  <Sparkles size={12} style={{ display: 'inline', marginRight: 4 }} />
                  {loading ? 'Generating...' : 'Generate 4 from Noah'}
                </button>
                <button className="btn secondary" onClick={draftContentPack} disabled={digesting}>
                  {digesting ? 'Drafting...' : 'Draft Gmail Pack'}
                </button>
              </div>
            </div>
            {status && <p style={{ color: '#666', fontSize: 11, margin: '8px 0 0' }}>{status}</p>}
            {digestPack && (
              <div className="cmo-hook-banner" style={{ marginTop: 12 }}>
                <div className="cmo-label">Latest operator pack</div>
                <p>{digestPack.title}</p>
                <div style={{ marginTop: 8, display: 'grid', gap: 4, fontSize: 11, color: '#aaa' }}>
                  <span><strong style={{ color: '#fff' }}>Hook:</strong> {digestPack.hook}</span>
                  <span><strong style={{ color: '#fff' }}>Duration:</strong> {digestPack.durationSeconds}s</span>
                  <span><strong style={{ color: '#fff' }}>Video prompt:</strong> {digestPack.videoPrompt}</span>
                  <span><strong style={{ color: '#fff' }}>Image prompt:</strong> {digestPack.imagePrompt}</span>
                </div>
              </div>
            )}
          </div>
